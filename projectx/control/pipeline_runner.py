...
def run_research_pipeline(output_dir: Path | str = "runs/latest") -> ProjectXRunResult:
    ...
    The supervised run now records both:
    - the MVP positive-geometry lifecycle slice, and
    - the realization-class comparison carrier for the current R2↔R3 objective.
    """
    ...
    combined_status = "passed" if monitor["status"] == "passed" and comparison_summary["status"] == "passed" else "failed"
