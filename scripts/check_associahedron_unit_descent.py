...
def cover_edges_by_single_chord_addition(faces: list[Face]) -> list[tuple[Face, Face]]:
    ...
    for face in faces:
        for other in faces:
            if len(other) == len(face) + 1 and face < other:
                difference = other - face
                if len(difference) == 1 and other in face_set:
...
def check_n(n: int) -> tuple[dict[str, int | bool], list[dict[str, object]]]:
    ...
    if summary["max_rank"] != expected_max_rank:
        counterexamples.append(
            {
                "n": n,
                "failure": "max_rank_mismatch",
