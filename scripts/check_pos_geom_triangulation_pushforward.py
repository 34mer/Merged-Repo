...
TARGET_ID = "CHK-POS-GEOM-TRIANGULATION-PUSHFORWARD"
...
REQUIRED_SOURCE_WINDOWS = [
    {
        "name": "triangulation_pushforward_methods",
        "terms": ["triangulation", "push-forward", "canonical forms"],
        "max_span_chars": 2400,
    },
]

REQUIRED_SOURCE_TERMS_ANYWHERE = [
    "triangulation",
    "push-forward",
    "non-overlapping",
    "canonical form",
...
class PosGeomTriangulationPushforwardResult:
    target_id: str
    checker: str
    status: str
...
def check_source_windows() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    failures: list[dict[str, Any]] = []
    summary: dict[str, Any] = {"source_windows": {}, "source_terms_anywhere": {}}
    ...
    for term in REQUIRED_SOURCE_TERMS_ANYWHERE:
        present = term.lower() in normalized
        summary["source_terms_anywhere"][term] = present
        if not present:
...
def run_check() -> PosGeomTriangulationPushforwardResult:
    failures: list[dict[str, Any]] = []
    ...
    support_summary: dict[str, Any] = {
        **claim_summary,
        **source_summary,
        "checked_interface": "source coverage for triangulation/push-forward as canonical-form construction methods",
        "allowed_inference": "triangulation and push-forward may be cited as positive-geometry canonical-form construction/readout methods",
        "blocked_inference": "Formation Medium dynamics, substrate realization, physical settling law, or computation-replacement claim",
    }

    return PosGeomTriangulationPushforwardResult(
        target_id=TARGET_ID,
        checker="pos_geom_triangulation_pushforward_source_guard_v1",
        ...
        status_boundary=(
            "PASS_SOURCE_COVERAGE means the POS_GEOM_2017 source text and extracted claim contain triangulation "
            "and push-forward language as methods for constructing canonical forms of positive geometries. "
            "It is not a mathematical proof, not a triangulation-independence theorem, not an S2* theorem, "
            "not substrate evidence, and not a physical settling-law result."
        ),
    )


def write_result(path: Path, result: PosGeomTriangulationPushforwardResult) -> None:
