...
from scripts.check_surf_binary_boundary import TARGET_ID, run_check, write_result


def test_surf_binary_boundary_source_guard_passes() -> None:
    result = run_check()
...
def test_surf_binary_boundary_artifact_shape(tmp_path: Path) -> None:
    result = run_check()
    path = tmp_path / "CHK-SURF-BINARY-BOUNDARY.json"
    write_result(path, result)
    data = json.loads(path.read_text(encoding="utf-8"))
    assert data["target_id"] == TARGET_ID
    assert data["checker"] == "surf_binary_boundary_source_guard_v1"
